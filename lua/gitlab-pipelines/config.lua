...
function M.project_names()
  local names = {}
  for name, _ in pairs(M.options.projects or {}) do
...
function M.resolve_project(name)
  ...
  if name and name ~= "" then
    ...
    project = vim.tbl_deep_extend("force", { name = name }, project)
  elseif M.options.project then
    ...
    if projects[cwd_name] then
      project = vim.tbl_deep_extend("force", { name = cwd_name }, projects[cwd_name])
    else
      local first_name = next(projects)
      if first_name then
        project = vim.tbl_deep_extend("force", { name = first_name }, projects[first_name])
      end
    end
  end
