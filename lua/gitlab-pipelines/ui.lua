...
function M.open(state, options)
	...
	if not valid_buf(state.buf) then
		...
		if options.open_key and options.open_key ~= "" then
			vim.keymap.set("n", options.open_key, function()
				M.open_under_cursor(state)
			end, { buffer = state.buf, nowait = true, desc = "Open pipeline/MR in browser" })
		end
	end
...
function M.open_under_cursor(state)
	if not valid_win(state.win) then
		return
	end

	local pos = vim.api.nvim_win_get_cursor(state.win)
	local line_number, col = pos[1], pos[2]

	local url = render.url_at({ links = state.links or {} }, line_number, col)
	if not url then
		vim.notify(
			"No pipeline or merge request under cursor",
			vim.log.levels.WARN,
			{ title = "gitlab-pipelines.nvim" }
		)
		return
	end
